...
#define QC_DEV_TYPE_TAC    0x08
#define QC_DEV_TYPE_EPM    0x09
#define QC_DEV_TYPE_LIBUSB 0x0A
...
#define DEV_CLASS_LIBUSB   0x00000020
...
#define DEV_PROTOCOL_PROMIRA                       0x90
...
typedef struct _CB_PARAMS
{
   ...
   PWCHAR DevName;   // device name for I/O (normally the symbolic name)
   ...
   PWCHAR SocVer;    // socver number from device
   ...
   PWCHAR ParentLocationInfomation; // physical location of parent device if present
...
typedef struct _DEV_PARAMS_N
{
   ...
   PVOID  SocVer;          // socver number from device, in unicode
   ULONG  SocverLen;       // in bytes
...
namespace QcDevice
{
   QCDEVLIB_API VOID SetLoggingCallback(Utils::QCD_LOGGING_CALLBACK Cb);
   ...
   QCDEVLIB_API VOID GetDriverVersion(VOID);
